...
<body class=" pb-28">
    ...
    <div class="max-w-screen-lg mx-auto">
        <div class="relative flex items-center justify-center py-3 border-b border-gray-300 font-bold">
            ...
            <h1 class="text-lg text-center">BMI</h1>
        </div>

        <div x-data="bmiTabs()" class="max-w-screen-lg mx-auto mt-6">
            ...
            <div class="mt-6">
                <div x-show="tab === 'week'" x-cloak>
                    ...
                    @endif
                    <canvas id="bmi-week-chart" data-labels='@json($weekDays)'
                        data-data='@json($weekBmis)' height="250" class="w-full mt-5 mx-auto"></canvas>

                    <div class="px-5 mt-5">
                        <p class="font-bold text-xl">{{ $weekLabels[0] }}～{{ end($weekLabels) }}の平均BMI</p>
                        <p class="mt-5 text-5xl text-right">{{ number_format($weekAverage, 2) }} </p>
                    </div>
                </div>
                ...
                <div x-show="tab === 'month'" x-cloak>
                    ...
                    @else
                        <p class="text-center text-gray-400">データがありません</p>
                    @endif
                    <canvas id="bmi-month-chart" data-labels='@json($monthLabels)'
                        data-data='@json($monthBmis)' height="250" class="w-full mt-5 mx-auto"></canvas>
                    <div class="px-5 mt-5">
                        <p class="font-bold text-xl">{{ $fullPeriodLabel }}の平均BMI</p>
                        <p class="mt-5 text-5xl text-right">{{ number_format($monthAverage, 2) }} </p>
                    </div>
                </div>
                ...
                <div x-show="tab === 'year'" x-cloak>
                    ...
                    @else
                        <p class="text-center text-gray-400">データがありません</p>
                    @endif
                    <canvas id="bmi-year-chart" data-labels='@json($yearDays)'
                        data-data='@json($yearBmis)' height="250" class="w-full mt-5 mx-auto"></canvas>
                    <div class="px-5 mt-5">
                        <p class="font-bold text-xl">{{ $yearLabels[0] }} ～ {{ end($yearLabels) }}の平均BMI</p>
